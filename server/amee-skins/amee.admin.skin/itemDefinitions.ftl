...
<script type="text/javascript">
    function deleteItemDefinition(itemDefinitionUid) {
        resourceUrl = '/admin/itemDefinitions/' + itemDefinitionUid + '?method=delete';
...
<p><a href='/admin'>Admin</a> /
   <a href='/admin/itemDefinitions'>Item Definitions</a></p>
        ...
        <#list itemDefinitions as id>
            <#if canViewEntity(id)>
                <tr id='Elem_${id.uid}'>
                    <td>${id.name}</td>
                    <td>
                        <a href='/admin/itemDefinitions/${id.uid}'><img src="/images/icons/page_edit.png" title="Edit" alt="Edit" border="0"/></a>
...
<#if canCreate()>
    ...
    <p>
        <form action='/admin/itemDefinitions' method='POST' enctype='application/x-www-form-urlencoded'>
