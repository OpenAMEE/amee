...
class ProfileIT extends BaseApiTestV1 {
    ...
    void createDeleteProfileJson() {
        ...
        assert config.api.standard.v1.user == responsePost.data.profile.user.username
    ...
    void createDeleteProfileXML() {
        ...
        def responsePost = client.post(
            ...
            contentType: XML)
        assert SUCCESS_OK.code == responsePost.status
        assert config.api.standard.v1.user == responsePost.data.ProfilesResource.Profile.User.Username.text()
    ...
    void getProfileCategoriesJson() {
        def responseGet = client.get(
            path: "/profiles/P3S1C0SF9T54/home",
            contentType: JSON)
        assert SUCCESS_OK.code == responseGet.status
    ...
    void getProfileCategoriesXML() {
        def responseGet = client.get(
            ...
            contentType: XML)
        assert SUCCESS_OK.code == responseGet.status
        assert responseGet.data.ProfileCategoryResource.Children.ProfileCategories.DataCategory.size() > 0
    }   
}
