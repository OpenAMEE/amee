...
<p><#if profileItemValue.value != ''>Value: ${profileItemValue.value}<br/></#if>
   ...
   Created: ${profileItemValue.created?string.short}<br/>
   Modified: ${profileItemValue.modified?string.short}<br/>
</p>
<#if browser.profileItemValueActions.allowModify>
  <h2>Update Profile Item Value</h2>
  <p>  <form action='${basePath}?method=put' method='POST' enctype='application/x-www-form-urlencoded'>
  Value:
  <#if profileItemValue.itemValueDefinition.choicesAvailable>
    <select name='value'>
    <#list profileItemValue.itemValueDefinition.choiceList as choice>
      <option value='${choice.value}'<#if profileItemValue.value == choice.value> selected</#if>>${choice.name}</option>
    </#list>
    </select>
  <#else>
