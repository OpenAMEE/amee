...
//if age=post 1995 and hasCavityWall=false then add "subtractCavityWall"
	if(hasCavityWallInsulation=="true")
		spaceSaveFac*=(1-cavityWall);
	else if(age.indexOf("post")>=0)
		spaceSaveFac*=(1-subtractCavityWall);
//if age=pre 1930 and hasSolidWall=true then add "solidWall", else 0.
...
function applyWaterRenewablesSavings(baseCO2){
	...
	if(hasSolarHotWater=="true")
		baseCO2-= solarHotWaterEnergySaved*waterEmissionFactor/12.;
	return baseCO2;
}
...
spaceCO2=getSpacekgCO2(efficiency)/12;
waterCO2=getWaterkgCO2(efficiency)/12;
